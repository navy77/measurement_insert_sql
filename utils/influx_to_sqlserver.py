...
class PREPARE:
    ...
    def __init__(self,server,database,user_login,password,table,table_log,table_columns,table_columns_log,columns_init
                 ,initial_db,influx_server,influx_database,influx_user_login,influx_password,influx_port,mqtt_topic):
        self.server = server
...
class INFLUX_TO_SQLSERVER(PREPARE):
    ...
    def query_sql(self):
        try:
            ...
            sql_query = f"""SELECT TOP 2000 * FROM [{self.database}].[dbo].[{self.table}] ORDER by registered_at desc"""
